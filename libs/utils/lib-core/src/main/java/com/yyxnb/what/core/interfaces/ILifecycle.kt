 ...
 *
 *
 * LifecycleObserver接口（ Lifecycle观察者）：实现该接口的类，通过注解的方式，可以通过被 [Lifecycle.addObserver] 方法注册,
 ...
 * [OnLifecycleEvent] 如：@OnLifecycleEvent(Lifecycle.Event.ON_CREATE)
 * [Lifecycle.Event]
 *
 *
 ...
 *
 *
 * State(当前生命周期所处状态)。[Lifecycle.getCurrentState]
 *
 *
 * Event(当前生命周期改变对应的事件)：当Lifecycle发生改变，如进入onCreate,会自动发出ON_CREATE事件， @OnLifecycleEvent(Lifecycle.Event.ON_CREATE)
 *
 *
 * 注册 [Lifecycle.addObserver]
 * 移除 [Lifecycle.removeObserver]
 * 当前状态 [Lifecycle.getCurrentState]
 *
 * @author yyx
 */
